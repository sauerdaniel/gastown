...
func startDeaconSession(t *tmux.Tmux, sessionName, agentOverride string) error {
	...
	// Auto-execute gt prime to trigger autonomous patrol execution.
	// Use NudgeSession instead of SendKeys for reliable command delivery.
	// Wait for Claude to be ready before sending command.
	time.Sleep(2 * time.Second)
	_ = t.NudgeSession(sessionName, "gt prime") // Non-fatal
